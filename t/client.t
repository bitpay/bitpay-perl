...
BEGIN{
    my $pos_token = "HwC497kDGihPFUDiPfbedj6etbrVHyKQaiFHPGspYHHV";
    ...
    my $getTokens = '{"data": [{"pos/invoice":"7zjXTgeWaTj7xXNBA1qV7iBGGHJZNiLjPXNquHdk8BUN"}, {"pos":"'.$pos_token.'"}]}';
    my $httpd = run_http_server {
        ...
        return do {
            if ($method eq "POST") {
                ...
                } elsif ($uri->path eq '/invoices/45678') {
                    return [
                        200,
                        [ 'Content-Type' => 'application/json' ],
                        [ '{"data":{"token":"'.$uri->query.'"}}']
                    ]
                ...
                } elsif ($uri->path eq '/malformedjson') {
                    [
                        500,
                        [ 'Content-Type' => 'application/json' ],
                        [ 'XXX' ]
    ...
    my %invoice = $client->get_invoice(id => 45678);
    is($invoice{token}, 'token='.$pos_token);

    throws_ok(sub { $client->get(path => 'malformedjson') }, qr/CONTENT:\nXXX/);
}
